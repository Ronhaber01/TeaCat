...
return e.category?.toLowerCase() === activeGenre?.toLowerCase() ||
        (e.vibe_tags ?? []).some((t: string) => t.toLowerCase() === (activeGenre ?? '').toLowerCase())
}
...
{bellToast && (
<span className="text-[#A3FF12] text-[10px] font-bold absolute right-14 top-5 pointer-events-none">
{bellActive ? 'Notifs on!' : 'Notifs off'}
</span>
)}
<button
onClick={handleBell}
