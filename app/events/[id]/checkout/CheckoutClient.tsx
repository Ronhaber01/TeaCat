        ...
        <div className="min-h-screen bg-[#111111] pb-10">
          {/* Header */}
              <div className="px-5 pt-14 pb-6">
                      ...
                      <h1 className="text-white font-black text-2xl mb-1">Get your ticket</h1>h1>
                      <p className="text-gray-500 text-sm">Tap to pay, show QR at the door.</p>p>
              </div>
        
          {/* Event summary */}
              <div className="mx-5 mb-6 p-4 rounded-2xl bg-[#1A1A1A] border border-[#2A2A2A]">
                      <h2 className="text-white font-bold text-base leading-tight mb-1">{event.title}</h2>h2>
                      <p className="text-gray-500 text-sm">{event.venue_name} · {event.neighborhood}</p>p>
                      <p className="text-gray-600 text-xs mt-1">{format(new Date(event.starts_at), 'EEEE, MMMM d · h:mm a')}</p>p>
                      <div className="mt-3 pt-3 border-t border-[#2A2A2A] flex items-center justify-between">
                                <span className="text-gray-400 text-sm">General · 1 ticket</span>span>
              ...
              <div className="px-5">
                ...
                {error && (
                    <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/30 mb-4">
                                <p className="text-red-400 text-sm font-semibold">{error}</p>p>
                    </div>div>
                      )}
                                                  ...
                                                  return (
                                                        <div className="flex flex-col gap-4">
                                                          {error && <p className="text-red-400 text-sm">{error}</p>p>}
                                                              ...
                                                              <p className="text-gray-700 text-xs text-center">No payment needed · Show QR at door</p>p>
                                                        </div>div>
                                                      )
                              }
</div>
