    ...
    if (userId) {
      try {
        ...
        const { data: ev } = await supabase
          .from('events').select('title, starts_at, venue_name').eq('id', eventId).single()
        if (owner?.email && ev) {
          const resend = new Resend(process.env.RESEND_API_KEY)
          const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${ticketCode}`
          const eventDate = ev.starts_at
            ? new Date(ev.starts_at).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
            : 'TBD'
          await resend.emails.send({
            from: 'TeaCat <onboarding@resend.dev>', to: owner.email,
            subject: `Your ticket to ${ev.title}`,
            html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px"><h1>You're going! 🎉</h1><p>Ticket: <strong>${ev.title}</strong></p><p><strong>Date:</strong> ${eventDate}</p><p><strong>Venue:</strong> ${ev.venue_name || 'TBD'}</p><p><strong>Amount paid:</strong> $${(pi.amount_received/100).toFixed(2)}</p><p><strong>Code:</strong> <code style="font-size:18px;letter-spacing:2px">${ticketCode}</code></p><div style="margin:24px 0"><img src="${qrUrl}" alt="QR" width="200"/></div><p style="color:#666">Show this QR at the door. — TeaCat</p></div>`,
          })
        }
