...
export async function POST(req: Request) {
  try {
    ...
    // Look up existing ticket by payment_intent_id
    const paymentKey = paymentIntentId || sessionId
    const { data: existingTicket } = await supabase
      .from('tickets')
      .select('id, ticket_code, event_id, status, events(title, date, venue)')
    ...
    if (!eventId) {
      return NextResponse.json({ error: 'Missing eventId in payment metadata' }, { status: 400 })
    }
    ...
    // Increment tickets_sold (SECURITY DEFINER, works with any authenticated caller)
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
            ...
            <div style="background: #f9f9f9; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
              ...
              <p style="margin: 0;"><strong>🎫 Ticket Code:</strong> <code style="background:#eee;padding:2px 6px;border-radius:4px;">${ticketCode}</code></p>
            </div>
