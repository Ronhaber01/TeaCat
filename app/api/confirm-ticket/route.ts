...
export async function POST(req: Request) {
  try {
    ...
    if (error) {
      // Race condition: edge function webhook created ticket between our check and insert
      if (error.code === '23505') {
        const { data: raceTicket } = await supabaseAdmin
          .from('tickets').select('*').eq('stripe_payment_id', paymentIntentId).single()
        if (raceTicket) return NextResponse.json({ ticket: raceTicket })
      }
