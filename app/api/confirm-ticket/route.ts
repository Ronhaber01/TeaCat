...
export async function POST(req: Request) {
  try {
    ...
    // Cookie-based client — read the authenticated user session
    const supabase = createClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })

    // Service role client — INSERT bypasses RLS (tickets created server-side only)
    ...
    // Idempotency check — webhook may have already created this ticket
    ...
    // Admin client so INSERT bypasses RLS — tickets are only created server-side
