  ...
  useEffect(() => {
    ...
    const fetchTickets = async () => {
      const { data } = await supabase
        .from('tickets')
        .select('*, event:events(id, title, venue_name, neighborhood, starts_at, category, flyer_url, host:hosts(logo_url, name))')
        .eq('user_id', user.id)
  ...
  return (
    <div className="min-h-screen bg-[#111111] pb-28">
      ...
      {shown.length === 0 ? (
        ...
        <>
          {currentSelected && (
            <div className="mx-5 mb-6 rounded-3xl overflow-hidden border border-[#2A2A2A] bg-[#1A1A1A]">
              {currentSelected.event?.flyer_url ? (
                <div className="relative w-full h-44 overflow-hidden">
                  <img
                    src={currentSelected.event.flyer_url}
                    alt={currentSelected.event.title}
                    className="w-full h-full object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-b from-black/20 to-[#1A1A1A]" />
                  {currentSelected.event.host?.logo_url && (
                    <div className="absolute top-3 left-3">
                      <img
                        src={currentSelected.event.host.logo_url}
                        alt={currentSelected.event.host.name}
                ...
                <div className="relative w-full h-20 bg-gradient-to-r from-[#7B2EFF]/40 to-[#7B2EFF]/10 flex items-center px-4 gap-3">
                  {currentSelected.event?.host?.logo_url && (
                    <img
                      src={currentSelected.event.host.logo_url}
                      alt={currentSelected.event.host.name}
                  ...
                  <span className="text-white/40 text-sm font-bold">
                    {currentSelected.event?.host?.name}
                  </span>
                </div>
              )}
              ...
              <div className="p-6 text-center">
                ...
                <p className="text-[#7B2EFF] font-black text-xl tracking-widest mb-1 font-mono">
              ...
              >
                <div className="flex items-center gap-4 p-4">
                  {ticket.event?.flyer_url ? (
                    <div className="flex-shrink-0 w-12 h-16 rounded-xl overflow-hidden">
                      <img src={ticket.event.flyer_url} alt="" className="w-full h-full object-cover" />
                    </div>
                  ) : (
